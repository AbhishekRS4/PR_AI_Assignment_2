...
for x in range(100):
    ...
    # Put all the fraud class in a separate data set.
    fraudulent_data = shuffled_data.loc[shuffled_data['Class'] == 1]

    # randomly sample the same amount of
    normal_data = shuffled_data.loc[shuffled_data['Class'] == 0].sample(n=len(fraudulent_data),random_state = randint(1,50))

    # Concatenate both data frames again
    data = pd.concat([fraudulent_data, normal_data])
    data = data.sample(frac=1,random_state=1)

    # create training data set as 80% of total data set
    x_train = data.sample(frac = 0.8, random_state = randint(1,50))

    # remaining 20% is test data
    x_test = data.drop(x_train.index)

    # divide training data into 30% labelled data and 70% unlabelled data
    x_train_lab = x_train.sample(frac=0.3, random_state = randint(1,50))
    ...
    # print('Accuracy score of the Logistic Regression model is {}'.format(accuracy_score(y_test, lr_test)))
    # print('F1 score of the Logistic Regression model is {}'.format(f1_score(y_test, lr_test)))
    ...
    # print the accuracy and F1 score
    # print('Accuracy score of the Label Propagation model is {}'.format(accuracy_score(y_test, lp_test)))
    # print('F1 score of the Label Propagation model is {}'.format(f1_score(y_test, lp_test)))
    ...
    lr2.fit(X_train_lp, y_train_lrlp)
    lrlp_test = lr2.predict(X_test)

    # print the accuracy and F1 score
    # print('Accuracy score of the Label Propagation model is {}'.format(accuracy_score(y_test, lrlp_test)))
    # print('F1 score of the Label Propagation model is {}'.format(f1_score(y_test, lrlp_test)))

    accuracy_lrlp.append(accuracy_score(y_test, lrlp_test))
    f1_score_lrlp.append(f1_score(y_test, lrlp_test))

# print(sum(accuracy_lp)/len(accuracy_lp))
...
print('Average accuracy of the Linear Regression model with Labels: ', sum(accuracy_lrlp)/len(accuracy_lrlp))
print('Average F1 score of the Linear Regression model with labels: ', sum(f1_score_lrlp)/len(f1_score_lrlp))

p.plot(range(100), accuracy_lr)
p.title('accuracy score over 100 runs of Linear Regression model')
p.xlabel('iteration')
...
p.title('accuracy score over 100 runs of Linear Regression model with learned labels')
p.xlabel('iteration')
p.ylabel('accuracy score')
p.show()
