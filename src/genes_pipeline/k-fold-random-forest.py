...
for i, n_components in enumerate(N_PC):
    ...
    for fold_position in range(K):
        ...
        rf = randomForest(training, n_trees = N_TREES ,bins = N_BINS, stoppingCriteria= "size", stoppingValue= STOPPING_VALUE)

        for value in testing:
            i1 = distinct_lables.index(value[-1])
            i2 = distinct_lables.index(rf.classify(value[:-1]))
            confusion_matrix[i1][i2] +=1

            if rf.classify(value[:-1]) == value[-1]:
                performance+= 1
